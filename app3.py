...
#dir1 = "/home/steviecurran/mysite/App3/"; dir2 = "/home/steviecurran/NZ_maps/"; dir3 = "http://steviecurran.pythonanywhere.com/static/images/"  # pythonanywhere
...
def donut_etc(check,date1,date2,slide_value,sector_radio,pc,g_total):
    ...
    if check == None or check == []:  
        ...
        date1 = dff['year'].iat[0]; 
        date2= dff['year'].iat[-1];
    ...
    #dfm1 = totals.copy();  # GIVES OPTION TO CHANGE - totals DOESN'T  HAVE INDUSTRY INFO

    if  sector_radio == 'All':
        dfm1 = dff.groupby(['region'])['Mtonnes'].sum().reset_index()
                  
    else:
        dfm1 = dff[dff['anzsic_descriptor'] == sector_radio]
    #print(totals); print(dfm1)
    ...
    temp = dfm1.groupby(['region'])['Mtonnes'].sum().reset_index() # NEED TO PRESERVE TITLES
    ...
    map1.layout.coloraxis.colorbar.title =  '%s - CO<sub>2</sub> equivalent' %(label_text)
    map1.update_layout(coloraxis_colorbar=dict(thicknessmode="pixels", thickness=20,
                                               lenmode="pixels",xanchor="left", x = 0.8,
                                               len=350,title_side = 'right',tickfont_size=14),
                                                margin={"pad": 0, "r":0,"t":0,"l":0,"b":0})
    ##############
    ...
    gas_totals = totals.groupby(['year','Gas'])[para].sum().reset_index();
